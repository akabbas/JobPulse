...
class WeWorkRemotelyScraper:
    ...
    def setup_logging(self):
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('logs/weworkremotely_scraper.log'),
    ...
    def search_jobs(self, keyword: str, location: str = "United States", limit: int = 100) -> List[Dict]:
        ...
        while len(jobs) < limit:
            try:
                ...
                self.logger.info(f"Searching We Work Remotely page {page + 1} for '{keyword}'")
                time.sleep(random.uniform(2, 4))
                response = self.session.get(search_url, params=params, timeout=30)
                response.raise_for_status()
                soup = BeautifulSoup(response.content, 'html.parser')
                job_cards = soup.find_all('li', {'class': 'feature'})
                if not job_cards:
                    self.logger.warning("No job cards found on We Work Remotely page")
                    break
                ...
                self.logger.error(f"Error scraping We Work Remotely page {page}: {str(e)}")
                break
        self.logger.info(f"Scraped {len(jobs)} jobs from We Work Remotely for '{keyword}'")
        return jobs
    
    def _extract_job_data(self, card, keyword: str) -> Optional[Dict]:
        try:
            # Job title
            title_elem = card.find('span', {'class': 'title'})
            ...
            # Job URL
            job_link = card.find('a', {'class': 'job-link'})
            ...
            snippet_elem = card.find('span', {'class': 'region'})
            ...
            self.logger.error(f"Error extracting We Work Remotely job data: {str(e)}")
            return None
